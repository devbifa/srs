...
<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class All_training_schedule extends MY_Controller {
...

		{
			...
			$data['data_ftd'] = $this->mymodel->selectWithQuery("SELECT a.id,a.date, CONCAT(h.serial_number) as ftd_model,a.eet_utc,a.etd_utc,a.eta,a.remark,a.status,f.nick_name as pic,g.nick_name as 2nd,d.course_code as course,c.batch,e.id as id_mission, CONCAT(e.subject_mission,'. ',e.name) as mission,e.name as mission_name
			...
			WHERE DATE(a.date_of_flight) >= '$start_date' AND DATE(a.date_of_flight) <= '$end_date' AND a.visibility = '1'
			"
			.$origin_base.
			"
			ORDER BY
			a.date_of_flight ASC, a.etd_utc ASC");
			$this->template->load('template/template','dashboard/all_training_schedule/all',$data);
		...
		function filter(){
			...
			redirect(base_url().'dashboard/all_training_schedule');
		}
