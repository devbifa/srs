...
class Daily_movement_report extends MY_Controller {
...

	{
		...
		$this->template->load('template/template','master/daily_movement_report/all-daily_flight_schedule',$data);
...
$this->form_validation->set_rules('dt[if]', '<strong>Instrument Flight</strong>', 'required');
}
...

	{
		...
		$data['daily_flight_schedule'] = $this->mymodel->selectDataone('daily_flight_schedule',array('id'=>$id));
		
		$data['file'] = $data['daily_flight_schedule']['file_report'];
	...
	{	
		...
		if ($this->form_validation->run() == FALSE){
			...
			if (!empty($_FILES['file']['name'])){
				...
				$dat = $this->mymodel->selectDataOne('syllabus_mission',array('code'=>$dt['mission'],'type_of_training'=>'FLIGHT'));
				$mission = $dat;
				...
				$course = $dat;
					...
					$mission = $this->mymodel->selectDataOne('syllabus_mission',array('code'=>$dt['mission']));
					$this->db->select('id,id_number,batch,nick_name');
					$student_1 = $this->mymodel->selectDataOne('user',array('id_number'=>$dt['pic']));
					$this->db->select('id,id_number,batch,nick_name');
					$student_2 = $this->mymodel->selectDataOne('user',array('id_number'=>$dt['2nd']));
					$type = 'FLIGHT';
					if($student_2){
						$config['file_name'] = $student_1['nick_name']."_".$student_2['nick_name']."_BATCH_".$dt['batch']."_".$course['code_name']."_".$type."_".$mission['code_name']."_".$id.".".$ext;
					}else{
						$config['file_name'] = $student_1['nick_name']."_BATCH_".$dt['batch']."_".$course['code_name']."_".$type."_".$mission['code_name']."_".$id.".".$ext;
					}
