	...
	$this->form_validation->set_rules('dt[role]', '<strong>Role</strong>', 'required');
		...
		public function store()
		{
	    	...
	    	if ($this->form_validation->run() == FALSE){
				...
				$dt['menu_sub'] = json_encode($_POST['menu_sub']);
				$dt['created_at'] = date('Y-m-d H:i:s');
				$dt['created_by'] = $_SESSION['id'];
				$dt['created_role'] = $_SESSION['role'];
				$dt['status'] = "ENABLE";
		...
		public function update()
		{
	    	...
	    	if ($this->form_validation->run() == FALSE){
				$this->alert->alertdanger(validation_errors());     
					...
					$dt['menu'] = json_encode($_POST['menu']);
					$dt['menu_sub'] = json_encode($_POST['menu_sub']);
					$dt['sub_role'] = json_encode($_POST['sub_role']);
					$dt['email_notification'] = json_encode($_POST['email_notification']);
...

		{
				...
				$file = $this->mymodel->selectDataone('file',array('table_id'=>$id,'table'=>'role'));

				@unlink($file['dir']);

				$this->mymodel->deleteData('file',  array('table_id'=>$id,'table'=>'role'));



				$str = $this->mymodel->deleteData('role',  array('id'=>$id));
