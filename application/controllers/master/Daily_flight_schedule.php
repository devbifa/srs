	...
	class Daily_flight_schedule extends MY_Controller {
		...
		function reset(){
			...
			if($start_date && $end_date){
				$this->mymodel->deleteData('daily_flight_schedule', "DATE(date_of_flight) >= '$start_date' AND DATE(date_of_flight) <= '$end_date' ".$origin_base." ");
			}else{
				$start_date = '1970-01-01';
				$this->db->delete('daily_flight_schedule', "origin_base = '$origin_base' AND DATE(date_of_flight) = '$start_date'");
			}
...

		{
			...
			if($_SESSION['start_date'] && $_SESSION['end_date']){
				...
				$this->template->load('template/template','master/daily_flight_schedule/add-daily_flight_schedule_empty',$data);
			}
