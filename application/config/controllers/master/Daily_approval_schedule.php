...
class Daily_approval_schedule extends MY_Controller {
...

	{
		...
		$this->template->load('template/template','master/daily_approval_schedule/all',$data);
	...
	function filter(){
		...
		redirect(base_url().'master/daily_approval_schedule');
	}
...
$this->form_validation->set_rules('dt[remark]', '<strong>Remark</strong>', 'required');
...

	{
		...
		$this->template->load('template/template','master/daily_approval_schedule/edit',$data);
