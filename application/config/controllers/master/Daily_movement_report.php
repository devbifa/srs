	...
	{	
		...
		if ($this->form_validation->run() == FALSE){
			...
			$ext = end($array);
				...
				$dir  = "webfile/report/flight/";

				$config['upload_path']          = $dir;

				$config['allowed_types']        = '*';

				$config['file_name']           = $dt['mission'].'_'.str_replace(' ','_',$dt['pic']).'_'.$dt['2nd'].'_'.strtoupper(DATE('d_M_Y', strtotime($dt['date_of_flight']))).'_'.$id.'.'.$ext;
			...
			if ( ! $this->upload->do_upload('file')){
					...
					$file = $this->mymodel->selectDataone('file',array('table_id'=>$id,'table'=>'daily_movement_schedule'));
