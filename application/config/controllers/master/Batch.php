...
$this->form_validation->set_rules('dt[description]', '<strong>Description</strong>', 'required');
...

		{
	        ...
	        $this->datatables->select('a.id,COUNT(c.id) as number_of_student, b.curriculum,a.code,a.batch,a.description,a.status');
	        ...
	        $this->datatables->join('curriculum b', 'a.curriculum = b.id', 'LEFT');
...

		{
			...
			$data['batch'] = $this->mymodel->selectDataone('batch',array('id'=>$id));$data['file'] = $this->mymodel->selectDataone('file',array('table_id'=>$id,'table'=>'batch'));$data['page_name'] = "batch";
