...
class My_report_approval extends MY_Controller {
...

	{
		...
		$this->template->load('template/template','menu/my_report_approval/all',$data);
	...
	function filter(){
		...
		redirect(base_url().'menu/my_report_approval');
	}

	function approve($id){
		$url = "http://localhost:1996/bifa/menu/my_approval/approve/$id";
	...
	function approve_report($id){
		$url = "http://localhost:1996/bifa/menu/my_approval/approve_report/$id";
		$this->template->curl_url($url);
		redirect(base_url().'menu/my_report_approval');
	}
...
$this->db->query("UPDATE daily_ftd_schedule SET visibility = '1' WHERE visibility = '0' ".$base.$date);
	...
	function approve_2($id){
		$url = "http://localhost:1996/bifa/menu/my_approval/approve_2/$id";
		$this->template->curl_url($url);
		redirect(base_url().'menu/my_report_approval');
	}
...
$this->form_validation->set_rules('dt[remark]', '<strong>Remark</strong>', 'required');
...

	{
		...
		$this->template->load('template/template','menu/my_approval/edit',$data);
