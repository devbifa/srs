          ...
          <div class="box">
            ...
            <div class="box-body">
              ...
              <table class="table table-bordered">
                <tr class="bg-success">
                <th style="width:20px;">NUM</th><th>APPROVAL FOR</th><th>USER</th>
                </tr>
                <?php
                  $data = $this->mymodel->selectWithQuery("SELECT * FROM authorize_approval");
                  foreach($data as $key=>$val){
                    
                    $text = "";
                   
                      $json_setting = json_decode($val['json_setting'],true);

                      $base = $this->mymodel->selectWithQuery("SELECT * FROM base_airport_document ORDER BY base ASC");
                      foreach($base as $key2=>$val2){
                        $base_detail = $this->mymodel->selectDataOne('base_airport_document',array('base'=>$val2['base']));
                        $user = $this->mymodel->selectDataOne('user',array('id'=>$json_setting[$val2['base']]['user']));
                        if($user){
                          $text .= $val2['base'].' : '.$user['full_name'].' <br>';
                        }else{
                          $text .= $val2['base'].' : - <br>';
                        }
                    ...
                    $dat = $this->mymodel->selectDataOne('role', array('id'=>$val['role']));
                    $val['role'] = $dat['role'];
                ?>
<tr>
<td><?=$key+1?></td>
<td class="text-left"><a href="<?=base_url()?>master/authorize_approval/edit/<?=$val['id']?>"><?=$val['val']?></a></td>
