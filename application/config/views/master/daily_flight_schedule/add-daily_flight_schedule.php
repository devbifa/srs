...
<select style='width:100%' name="dt[batch]" class="form-control select2" id="batch">
 <option value="">SELECT BATCH</option>
  ...
  foreach ($base_airport_document as $base_airport_document_record) {
    ...
    if($base_airport_document_record['batch']==$daily_flight_schedule['batch']){
    ...
    echo "<option value='".$base_airport_document_record['batch']."' ".$text." >".$base_airport_document_record['batch'].' (TPM '.$base_airport_document_record['curriculum'].")</option>";
...
<select class="form-control select2" id="tpm" placeholder="Masukan TPM" name="dt[tpm]">
  <option value="<?= $daily_flight_schedule['tpm'] ?>"><?= $daily_flight_schedule['tpm'] ?></option>
</select>
...
<button type="submit" class="btn btn-primary btn-send pull-right" ><i class="fa fa-save"></i>  SAVE</button>
...
<div class="col-md-12">
  ...
  <div class="">
   ...
   <div class="row">
                            <div class="">
                                <form autocomplete="off" action="<?= base_url() ?>master/daily_flight_schedule/filter_create" method="post">
                                    <div class="col-md-2">
...
<button type="button" class="btn btn-block btn-warning"><i class="mdi mdi-arrow-up-bold-circle-outline"></i> EXPORT DATA</button> 
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>&nbsp</label><br>
                                            <button type="button" class="btn  btn-block  btn-info" onclick="$('#modal-impor').modal()"><i class="mdi mdi-arrow-down-bold-circle-outline"></i> IMPORT DATA</button>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <form action="<?= base_url('fitur/import/daily_flight_schedule') ?>" method="POST"  enctype="multipart/form-data">
...
<tr class="bg-success">
<th style="width:20px">NUM</th><th style="min-width:100px">ACTION</th>
<th style="min-width:110px">DATE OF<br>FLIGHT</th><th>ORIGIN<br>BASE</th><th>AIRCRAFT<br>REG</th><th>PIC</th><th>2ND</th>
<th>BATCH</th><th>COURSE</th><th>MISSION</th><th>DEP</th><th>ARR</th><th>ROUTE</th><th>ETD<br>UTC</th><th>ETA<br>UTC</th><th>EET</th><th>REMARK</th><th>DUTY INSTRUCTOR</th><th>PROPOSE</th>
</tr>
</thead>
...
WHERE a.visibility = '0' AND DATE(a.date_of_flight) >= '$start_date' AND DATE(a.date_of_flight) <= '$end_date'  AND etd_utc >= '22:00' AND etd_utc <= '24:00'
AND a.type = '' "
.$origin_base.
"
GROUP BY a.id
...
foreach($data as $key=>$val){
 ...
 // $dat = $this->mymodel->selectDataOne('user',array('id_number'=>$val['pic']));
  ...
  <tr>
    <td><?=$nomor?></td>
    <td><a href="<?=base_url()?>master/daily_flight_schedule/edit/<?=$val['id']?>" class="btn btn-primary btn-xs btn-rounded"><i class="mdi mdi-pencil"></i></a>
     <a href="#!" data-toggle="modal" data-target="#modal-delete-<?=$val['id']?>" class="btn btn-danger btn-rounded btn-xs"><i class="mdi mdi-delete"></i></a>
...
<div class="modal modal-danger fade" id="modal-delete-<?=$val['id']?>">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body" style="color:#fff!important;">
               <form action="">
                <span style="font-weight:100">Are you sure you want to delete this schedule?</span>
               </form>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                <a type="button" class="btn btn-outline" href="<?=base_url()?>master/daily_flight_schedule/delete_data/<?=$val['id']?>">Delete Now</a>
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
  ...
  <td><?=DATE('d M Y',strtotime($val['date_of_flight']))?></td>
    ...
    <td><?=$val['eta_utc']?></td>
    <td><?=$val['eet']?></td>
    <td class="text-left"><?=$val['remark']?></td><td class="text-left"><?=$val['duty_instructor']?></td><td class=""><?=$sent?></td>
  </tr>
<?php } ?>
...
$data = $this->mymodel->selectWithQuery("SELECT *
FROM daily_flight_schedule a
WHERE a.visibility = '0' AND DATE(a.date_of_flight) >= '$start_date' AND DATE(a.date_of_flight) <= '$end_date' AND etd_utc >= '00:00' AND etd_utc <= '21:59'
AND a.type = '' "
.$origin_base.
"
GROUP BY a.id
...
<a href="#!" data-toggle="modal" data-target="#modal-submit" id="button-submit" class="btn btn-primary pull-right" ><i class="mdi mdi-send"></i> PROPOSE FOR APPROVAL</a>
<a href="#!" data-toggle="modal" data-target="#modal-delete" id="" class="btn btn-danger pull-left" ><i class="mdi mdi-delete"></i> RESET SCHEDULE</a>
...
<div class="modal modal-success fade" id="modal-submit">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                <a type="button" class="btn btn-outline" href="<?=base_url()?>master/daily_flight_schedule/submit" id="submit-now" >Propose Now</a>
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
        ...
        <div class="modal modal-success fade" id="modal-delete">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                <a type="button" class="btn btn-outline" href="<?=base_url()?>master/daily_flight_schedule/reset" id="submit-now" >Reset Now</a>
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
                   ...
                   var str = response;
                    ...
                    if (str.indexOf("success") != -1){
                        ...
                        setTimeout(function(){ 
                           ...
                           window.location.href = "<?= base_url('master/daily_flight_schedule/create') ?>";
