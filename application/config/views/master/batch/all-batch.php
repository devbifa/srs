...
<button type="button" class="btn btn-sm btn-success float-1"  data-placement="top" title="ADD BATCH" ><i class="mdi mdi-plus"></i></button> 
...
<tr class="bg-success">
  ...
  <th style="width:20px">NUM</th><th>BATCH</th><th>NUMBER OF STUDENT</th><th>CURRICULUM</th><th>STATUS</th><th>OPEN DATE</th>
...
FROM batch a
-- LEFT JOIN curriculum b
-- ON a.curriculum = b.id
...
<td><a title="EDIT BATCH" href="<?=base_url()?>master/batch/edit/<?=$val['id']?>"><?=$val['batch']?></a>
</td>
<td><a title="STUDENT LIST" href="<?=base_url()?>master/batch/student/<?=$val['batch']?>"><?=$val['number_of_student']?></a>
</td>
<td><?=$val['curriculum']?>
</td>
<td><?=$val['status']?>
