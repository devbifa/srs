...
$ground = $this->mymodel->selectWithQuery("SELECT * FROM daily_ground_schedule a
...
foreach($ground as $k=>$v){
  if (strpos($v['duration_act'], ':') !== false) {
    array_push($ground_act,$v['duration_act']);
    }  
}
...
ON a.ftd_model = b.id
WHERE a.visibility = '1' AND a.visibility_report = '1' AND  (a.pic = '$id_instructor' OR a.2nd = '$id_instructor') AND a.block_time_total NOT IN ('','-','00:00','0:00') AND b.type_enginee='SE'
...
LEFT JOIN synthetic_training_devices_document b
ON a.ftd_model = b.id
WHERE a.visibility = '1' AND a.visibility_report = '1' AND  (a.pic = '$id_instructor' OR a.2nd = '$id_instructor') AND a.block_time_total NOT IN ('','-','00:00','0:00') AND b.type_enginee='ME'
AND DATE(date) >='$start_date' AND DATE(date) <='$end_date'");
...
WHERE  a.visibility = '1' AND a.visibility_report = '1' AND (a.pic = '$id_instructor' OR a.2nd = '$id_instructor') AND a.block_time_total NOT IN ('','-','00:00','0:00') AND b.type='SE'
...
LEFT JOIN aircraft_document b
ON a.aircraft_reg = b.serial_number
WHERE  a.visibility = '1' AND a.visibility_report = '1' AND (a.pic = '$id_instructor' OR a.2nd = '$id_instructor') AND a.block_time_total NOT IN ('','-','00:00','0:00') AND b.type='ME'
AND DATE(date_of_flight) >='$start_date' AND DATE(date_of_flight) <='$end_date'");
...
$ground_act = $this->template->sum_time_3($ground_act);
...
<?php
array_push($ground_summary,$ground_act);
...
<?php } 
$ground_summary = $this->template->sum_time_3($ground_summary);
...
$total_ground_summary = $ground_summary;
...
$total_summary = array();
$total_summary[0] = $flight_me_summary;
