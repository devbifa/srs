...
$b = "show";
$c = "show";
if($_SESSION['type']=='GROUND'){
  $b = "hide";
  $c = "hide";
}else if($_SESSION['type']=='FTD'){
  $a = "hide";
  $c = "hide";
 ...
 <!-- Content Wrapper. Contains page content -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <table style="width:100%;">
              ...
              <tr>
                <th colspan="3" style="font-size:15px;padding:5px 0px;" class="text-left">CAPABILITY</th>
                <th> : </th>
                <?php
                $type_text = '';
                $type = ($student['type']);
      ...
      <div class="row">
                            <div class="">
                                <form autocomplete="off" action="<?= base_url() ?>record/instructor_hours/filter_detail/<?=$student['id']?>" method="post">
                                    <div class="col-md-2">
            ...
            <thead>
            <tr class="bg-orange">
                <th colspan="14" style="font-size:15px;" class="text-left"> <label>GROUND INSTRUCTOR HOURS</label>
                </th>
              </tr>
...
<th rowspan="2" >COURSE</th><th rowspan="2" >SUBJECT</th><th colspan="3">TIME (UTC)</th><th colspan="2" >PARTICIPANT</th> <th rowspan="2" >REMARK</th><th rowspan="2" >IRREG<br>CODE</th>
 </tr>
...
<tbody>
<?php 
$data = $this->mymodel->selectWithQuery("SELECT a.student, a.student_attend, a.student_other, a.student_other_attend, a.id,a.date,g.batch, CONCAT(c.base,' (',b.classroom,')') as classroom,d.course_code as course,e.name as subject,f.nick_name as instructor,a.duration,a.start_lt,a.stop_lt,a.remark,a.status
FROM
...
foreach($data as $key=>$val){
  ...
  if (strpos($val['duration'], ':') !== false) {
    ...
    if(empty($val['remark_report'])){
      array_push($total_actual,$val['duration']);
    }
  }
  $participant = 0;
  $attend = 0;
...
<tr>
  ...
  <td><?=DATE('d M Y',strtotime($val['date']))?></td><td><?=$val['classroom']?></td> <td class="text-left"><?=$val['instructor']?></td>  <td><?=$val['batch']?></td> <td><?=$val['course']?></td> <td class="text-left"><?=$val['subject']?></td> <td><?=$val['start_lt']?></td> <td><?=$val['stop_lt']?></td><td><?=$val['duration']?></td> 
  <td><?=$participant?></td> 
  <td><?=$attend?></td> 
...
<table>
  <tr>
    ...
    <p><?=$total_actual?></p>
            ...
            <thead>
            <tr class="bg-orange">
                <th colspan="16" style="font-size:15px;" class="text-left"> <label>FTD INSTRUCTOR HOURS</label>
                </th>
              </tr>
...
foreach($data as $key=>$val){
  ...
  if (strpos($val['eta'], ':') !== false) {
    array_push($total,$val['eta']);
  }
...
<table>
  <tr>
    ...
    <p><?=$total_block_time?></p>
            ...
            <thead>
            <tr class="bg-orange">
                <th colspan="25" style="font-size:15px;" class="text-left"> <label>FLIGHT INSTRUCTOR HOURS</label>
                </th>
              </tr>
...
<tbody>
<?php 
$data = $this->mymodel->selectWithQuery("SELECT a.id,a.block_time_start, a.block_time_stop,
a.block_time_total,
...
WHERE a.visibility = '1'  AND (a.pic = '$pic' OR a.2nd = '$pic' OR a.duty_instructor = '$pic') AND DATE(date_of_flight) >= '$start_date' AND DATE(date_of_flight) <= '$end_date'
