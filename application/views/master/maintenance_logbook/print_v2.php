  ...
  <table>
    <tr>
      ...
      <td colspan="10" class="text-center no-border" style="vertical-align:middle;">
        ...
        <h3 style="margin-top:10px;">AIRCRAFT REGISTRATION <?=$data['aircraft_reg']?></h3>
      </td>
      <td colspan="3" class="text-left no-border" style="width:200px;vertical-align:middle;">
        <p><b>LOG SHEET NO <?=$data['log_sheet']?></b></p>
        <p style="margin-top:10px;"><b>DATE <?=strtoupper(DATE('d M Y',strtotime($data['date'])))?></b></p>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td rowspan="7" colspan="4" class="text-left">
        ...
        if($data['release_status']=="RELEASE"){
          if($data['mr_created_at']){
            $data['mr_created_at_date'] = DATE('d M Y',strtotime($data['mr_created_at']));
            $data['mr_created_at_time'] = DATE('H:i',strtotime($data['mr_created_at']));
          }else{
          ...
          if($data['mr_ac_ttis_until']){
  
          }else{
            $data['mr_ac_ttis_until'] = '-';
          }
  
          if($data['mr_day_until']){
          ...
          if($data['mr_amel_no']){
            $data['mr_amel_no'] = $data['mr_amel_no'];
          }else{
            $data['mr_amel_no'] = '......................';
          }

          if($data['mr_created_by']){
            $arr = array();
            $arr = $this->mymodel->selectDataOne('user',array('id'=>$data['mr_created_by']));
            $data['mr_created_by'] = $arr['nick_name'];
            if(empty($arr)){
              $data['mr_created_by'] = '......................';
            }
          }else{
        ...
        if($data['di_created_at']){
          $data['di_created_at_date'] = DATE('d M Y',strtotime($data['di_created_at']));
          $data['di_created_at_time'] = DATE('H:i',strtotime($data['di_created_at']));
        }else{
          $data['di_created_at_date'] = '......................';
          $data['di_created_at_time'] = '......................';
        }

        if($data['di_created_by']){
          $arr = array();
          $arr = $this->mymodel->selectDataOne('user',array('id'=>$data['di_created_by']));
          $data['di_created_by'] = $arr['nick_name'];
          if(empty($arr)){
            $data['di_created_by'] = '......................';
          }
        }else{
        ...
        if($data['di_amel_no']){
          $data['di_amel_no'] = $data['di_amel_no'];
        }else{
          $data['di_amel_no'] = '......................';
        }
    ...
    <tr>
      ...
      <td colspan="2">
        <?=$data['bf_ac_hours_ttis']?>
      </td>
      <td colspan="2">
        <?=$data['bf_no_1_engine_tso']?>
      </td>
      <td colspan="2">
      <?php if($aircraft['type']=="SE"){ ?>
        -
      <?php }else{ ?>
        <?=$data['bf_no_2_engine_tso']?>
      <?php } ?>
      </td>
      <td colspan="2">
        <?=$data['bf_no_1_prop_tso']?>
      </td>
      <td colspan="2">
        <?php if($aircraft['type']=="SE"){ ?>
          -
        <?php }else{ ?>
          <?=$data['bf_no_2_prop_tso']?>
        <?php } ?>
      </td>
      ...
      <td rowspan="3" style="vertical-align:middle">
        <!-- <?=$data['ni_ac_ttis']?> -->
        <?=number_format($data['ni_ac_ttis'] -  (($data['hobbs_end_start'] - $data['aircraft_duration'])+$data['bf_ac_hours_ttis']),1)?>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td colspan="2">
        <?=$data['hobbs_end_start'] - $data['aircraft_duration']?>
      </td>
      <td colspan="2">
      <?=$data['hobbs_end_start'] - $data['aircraft_duration']?>
      </td>
    ...
    <tr>
      ...
      <td colspan="2">
        <?=($data['hobbs_end_start'] - $data['aircraft_duration'])+$data['bf_ac_hours_ttis']?>
      </td>
      <td colspan="2">
        <?=($data['hobbs_end_start'] - $data['aircraft_duration'])+$data['bf_no_1_engine_tso']?>
      </td>
      <td colspan="2">
      ...
      <?php if($aircraft['type']=="SE"){ ?>
        -
      <?php }else{ ?>
        <?=($data['hobbs_end_start'] - $data['aircraft_duration'])+$data['bf_no_2_engine_tso']?>
      <?php } ?>
       
      </td>
      <td colspan="2">
        <?=($data['hobbs_end_start'] - $data['aircraft_duration'])+$data['bf_no_1_prop_tso']?>
      </td>
      <td colspan="2">
      ...
      <?php if($aircraft['type']=="SE"){ ?>
        -
      <?php }else{ ?>
        <?=($data['hobbs_end_start'] - $data['aircraft_duration'])+$data['bf_no_2_prop_tso']?>
      <?php } ?>
      </td>
    </tr>
    ...
    $text_rectifation = '';
    $id = $data['id'];
    $arr_issue = $this->mymodel->selectWithQuery("SELECT * FROM maintenance_issue 
    WHERE id_maintenance = '$id' AND status = 'ENABLE' ORDER BY id ASC");
    foreach($arr_issue as $k=>$v){

      $this->db->select('nick_name');
      $usr  = $this->mymodel->selectDataOne('user',array('id'=>$v['d_created_by']));
      $text_usr_1 .= $usr['nick_name'].'<br><br>';

      $this->db->select('nick_name');
      $usr  = $this->mymodel->selectDataOne('user',array('id'=>$v['r_created_by']));
      $text_usr_2 .= $usr['nick_name'].'<br><br>';

      if(!in_array($v['discrepancies'],array("-",""))){
        ...
        $text_discrepancies .= '-<br><br>';
        $text_rectifation .= '-<br><br>';
      }
    ...
    $arr_oil = array();
    $id = $data['id'];
    $arr_oil = $this->mymodel->selectWithQuery("SELECT * FROM oil 
    WHERE id_maintenance = '$id' AND status = 'ENABLE' ORDER BY id ASC");
    ...
    <tr>
      <td colspan="6" rowspan="<?=$hobbs_rowspan?>">
        ...
        <br>
        <br>
        <p class="text-left">START<span style="margin-left:29.5px"> : <?= $data['hobbs_start'] ?></span></p>
        <p class="text-left">END<span style="margin-left:43px"> : <?= $data['hobbs_end'] ?></span></p>
        <p class="text-left">TOTAL<span style="margin-left:29.5px"> : <?= $data['hobbs_end_start'] ?></span></p>
        <br>
        <br>
        <br>
        <br>
        ...
        <div style="width:100%;display: table">
          ...
          <div style="width:49%;display:inline-block;text-align:right">
            <p><?= $data['hobbs_end_start'] - $data['aircraft_duration'] ?> NWS</p>
          </div>
        </div>
      </td>
      <td colspan="12">
