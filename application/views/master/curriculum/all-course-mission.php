...
if($uris=='course_mission'){
  ...
  $link_export = base_url().'master/curriculum/course_mission_export/'.$curriculum['code'].'/'.$types;
  $link_import = base_url().'master/curriculum/course_mission_import/'.$curriculum['code'].'/'.$types;
}else{
  $link_export = base_url().'master/curriculum/course_mission_custom_export/'.$curriculum['code'].'/'.$types;
  $link = base_url().'master/curriculum/course_mission_custom_print/'.$curriculum['code'].'/'.$types;
  $link_import = base_url().'master/curriculum/course_mission_custom_import/'.$curriculum['code'].'/'.$types;
}
?>

<a target="_blank" href="<?=$link?>" class="btn btn-default pull-right" style="margin-right:10px;">PRINT</a>

<a target="_blank" href="#!" data-toggle="modal" data-target="#modal-import" class="btn btn-default pull-right" style="margin-right:10px;">IMPORT</a>

<a target="_blank" href="<?=$link_export?>" class="btn btn-default pull-right" style="margin-right:10px;">EXPORT</a>

<div class="modal modal-warning fade" id="modal-import">
           <div class="modal-dialog">
             <div class="modal-content">
               <div class="modal-header">
                 <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                   <span aria-hidden="true">&times;</span></button>
                 <h4 class="modal-title">IMPORT MISSION</h4>
               </div>
               <div class="modal-body" style="color:#fff!important;">
                <form  enctype="multipart/form-data" action="<?=$link_import?>" method="POST">
                 <span style="font-weight:100">Are you sure you want to import this mission data?</span>
                <br>
               ...
               <div class="modal-footer">
                 ...
                 <button type="submit" class="btn btn-outline">Import Now</button>
               </div>
               </form>
             </div>
             <!-- /.modal-content -->
            ...
            <div class="box-body">
              ...
              <tr class="<?=$bg?>">
                 <th style="width:50px;"><a style="margin-top:0px;" href="<?=base_url()?>master/tpm_syllabus_all_course/create/<?=$curriculum_id?>/<?=$val['id']?>/<?=$type?>" class="btn-block btn btn-primary"><i class="mdi mdi-plus"></i></a>
                  ...
                  <th colspan="11"><?=$val['course_code']?> - <?=$val['course_name']?>
                  </th>
                ...
                foreach($mission as $key2=>$val2){
                  ...
                  if (strpos($val2['duration_non_rev'], ':') !== false) {
                    array_push($total_non_rev,$val2['duration_non_rev']);
                  }
...
<div class="modal modal-danger fade" id="modal-delete-1-<?=$val2['id']?>">
          ...
          <!-- /.modal-dialog -->
               ...
               $total_non_rev = $this->template->sum_time($total_non_rev);
                ...
                $curriculum = $val['curriculum'];
                // $this->db->order_by("position ASC");
                // echo "SELECT * FROM tpm_syllabus_all_course WHERE course = '$course' AND type_of_training = '$type' AND curriculum = '$curriculum' ORDER BY position ASC";
                $mission = $this->mymodel->selectWithQuery("SELECT * FROM tpm_syllabus_all_course WHERE course = '$course' AND type_of_training = '$type' AND curriculum = '$curriculum' ORDER BY position ASC");
              //  print_r($mission);
