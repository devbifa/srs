...
<label for="form-role">ROLE</label>
                      <select name="dt[role]" class="form-control select2"  style='width:100%'>
...
<label for="form-base">BATCH</label>
<select style='width:100%' name="dt[batch]" class="form-control select2">
  ...
  foreach ($base_airport_document as $base_airport_document_record) {
    ...
    if($base_airport_document_record['batch']==$user['batch']){
...
<label for="form-remark">EMAIL NOTIFICATION</label>
<br>
<?php 
$email_notification = json_decode($user['email_notification'],true);
...
foreach($data as $key=>$val){
  $text = ""; 
  if($email_notification[$val]==$val){
    $text = "checked";
  }
?>
<input name="dttt[<?=$val?>]" type="checkbox" <?=$text?> value="<?=$val?>" > <?=$val?> <br>
<?php } ?>
