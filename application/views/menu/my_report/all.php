      ...
      <div class="box">
        ...
        <div class="box-body">
            ...
            <div class="show_error"></div>
               ...
               <div class="row">
                        <div class="">
                            <form autocomplete="off" action="<?= base_url() ?>menu/my_report/filter" method="post">
                            <div class="col-md-2">
...
<thead>
<tr class="bg-orange">
    <th colspan="18" style="font-size:15px;" class="text-left"> <label>GROUND REPORT</label>
    </th>
  </tr>
...
if($_SESSION['instructor_status']=="1"){
  ...
  AND a.visibility NOT IN ('-','','00:00','0:00') AND instructor = '$pic'
  ...
  daily_ground_schedule a
  WHERE a.visibility = '1' AND a.visibility_report = '1' AND DATE(date) >= '$start_date' AND DATE(date) <= '$end_date'
  AND a.duration_act NOT IN ('-','','00:00','0:00') AND (student LIKE '%$qry%' OR student_other LIKE '%$qry%')
  ORDER BY a.date ASC,a.start_act ASC");
}
...
<td><?=DATE('d M Y',strtotime($val['date']))?></td><td><?=$val['classroom']?></td> <td class="text-left"><?=$val['instructor']?></td>  <td><?=$val['batch']?></td>
<td><?=$val['tpm']?></td> <td><?=$val['course']?></td> <td class="text-left">
  <?php 
  ...
  ?>  
  <?=$val['subject']?></td> 
  <td><?=$val['start_lt']?></td> <td><?=$val['stop_lt']?></td><td><?=$val['duration']?></td> <td><?=$val['start_act']?></td> <td><?=$val['stop_act']?></td><td><?=$val['duration_act']?></td> 
  <td><?=$participant?></td> 
  <td><?=$attend?></td> 
...
<thead>
<tr class="bg-orange">
    <th colspan="16" style="font-size:15px;" class="text-left"> <label>FTD REPORT</label>
    </th>
  </tr>
...
<thead>
<tr class="bg-orange">
    <th colspan="21" style="font-size:15px;" class="text-left"> <label>FLIGHT REPORT</label>
    </th>
  </tr>
